...
export class SolarPanelSystemWebSocketService {
  ...
  connect(deviceId: number, onMessage: (message: string) => void): void {
    const socket: WebSocket = new WebSocket(`${environment.socketUrl}/solar-panel-system-measurements?deviceId=${deviceId}`);
